...
const { Joi, buildErrorObject, csrfTokenSchema } = require('./validator');
...
const userSchema = new Schema({
    ...
    role: {
        ...
        validate: {
            validator: (val) => {
                for (let i = 0; i < schemaDefaults.role.values.length; i++) {
                    if (val === schemaDefaults.role.values[i] || val === '') {
                        return true;
                    }
                }
            ...
            message: `Role must be one of: "${schemaDefaults.role.values.join(
                '", "'
            )}"`
        }
    }
...
userSchema.statics.getAvailableRoles = function() {
    ...
    for (let i = 0; i < schemaDefaults.role.values.length; i++) {
        let role = {
            name: schemaDefaults.role.values[i],
            value: schemaDefaults.role.values[i].toLowerCase()
        };
...
userSchema.statics.validateLogin = function(data) {
    ...
    if (result.error) {
        result.error = buildErrorObject(result.error.details);
